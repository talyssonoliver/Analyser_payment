...
import {
  ...
  type FileInfo,
} from "@/lib/domain/services/file-fingerprint-service";
...
describe("FileFingerprintService - Dual Fingerprint System", () => {
  ...
  describe("Modern Fingerprint (SHA-256)", () => {
    ...
    it("should generate different fingerprint for different files", async () => {
      const files1: FileInfo[] = [{ name: "test1.pdf", size: 1024, lastModified: 1234567890 }];

      const files2: FileInfo[] = [{ name: "test2.pdf", size: 2048, lastModified: 1234567891 }];
  ...
  describe("Legacy Fingerprint (Base64 + 32-bit hash)", () => {
    it("should generate legacy fingerprint alongside modern", async () => {
      const files: FileInfo[] = [{ name: "test.pdf", size: 1024, lastModified: 1234567890 }];
    ...
    it("should match legacy algorithm behavior", () => {
      ...
      const fileInfo = [{ name: "test.pdf", size: 1024, lastModified: 1234567890 }];
    ...
    it("should truncate legacy fingerprint to 64 characters", async () => {
      const files: FileInfo[] = [
        {
          name: "very-long-filename-that-might-produce-a-long-base64-string.pdf",
          size: 999999,
  ...
  describe("Backward Compatibility", () => {
    ...
    it("should allow comparison with legacy fingerprints", async () => {
      ...
      const isModernSimilar = service.areSimilar(result.fingerprint, result.fingerprint);
...
describe("FileFingerprintService - Manual Entries", () => {
  ...
  it("should generate same fingerprint for identical manual entries", async () => {
    const data = {
      ...
      startDate: new Date("2024-01-01"),
      endDate: new Date("2024-01-07"),
      entries: [{ date: new Date("2024-01-01"), consignments: 50, paidAmount: 100 }],
    };
