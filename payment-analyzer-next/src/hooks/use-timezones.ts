...
export function useTimezones(options: UseTimezonesOptions = {}): UseTimezonesReturn {
  ...
  const fetchTimezones = useCallback(
    async (term: string = searchTerm): Promise<string[]> => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        switch (mode) {
          case "common": {
            ...
            const { data: commonData, error: commonError } =
              await supabase.rpc("get_common_timezones");
          ...
          default: {
            ...
            const { data: allData, error: allError } = await supabase.rpc("get_timezone_names");
  ...
  const search = useCallback(
    async (term: string): Promise<string[]> => {
      ...
      try {
        const { data, error } = await supabase.rpc("search_timezones", { search_term: term });
        ...
        return fallbackTimezones.filter((tz) => tz.toLowerCase().includes(term.toLowerCase()));
      }
    },
