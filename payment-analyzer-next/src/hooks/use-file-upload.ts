...
export function useFileUpload(config: UseFileUploadConfig) {
    ...
    (files: FileList | File[]): { valid: File[]; errors: string[] } => {
      ...
      fileArray.forEach((file) => {
        ...
        if (file.size > maxFileSize) {
          errors.push(
            `${file.name}: File size exceeds ${Math.round(maxFileSize / (1024 * 1024))}MB limit`
          );
          return;
        }

        // Check for duplicates in current session (same file uploaded twice)
        // Note: We only check in-memory duplicates here. Database duplicate checking
        // happens in Step1Container with smart overlap detection based on date ranges
        if (
          uploadedFiles.some(
            (uploaded) =>
          ...
          errors.push(
            `${file.name}: This file is already selected. Please choose a different file or remove the existing one first.`
          );
          return;
        }
   ...
   */
  const markFilesAsSuccess = useCallback(
    (newFiles: UploadedFile[]) => {
      const newFileIds = new Set(newFiles.map((f) => f.id));
      setUploadedFiles((prev) =>
        prev.map((f) => (newFileIds.has(f.id) ? { ...f, status: "success", progress: 100 } : f))
      );
    },
    [setUploadedFiles]
  );
    ...
    (e: React.DragEvent) => {
      ...
      if (errors.length > 0) {
        ...
        errors.forEach((error) => {
          console.error(error);
          toast.error(error); // Show toast notification for each error
        });
        return;
      }
