...
const filterDaysByAmount = (days: DayEntry[], amountFilter: string) => {
  return days.filter((day) => {
    switch (amountFilter) {
      case "high":
        return day.totalAmount >= 100;
      case "medium":
        return day.totalAmount >= 50 && day.totalAmount < 100;
      case "low":
        return day.totalAmount < 50;
      default:
        return true;
    }
  });
...
const compareDaysByDate = (a: DayEntry, b: DayEntry) =>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">
                  {month.totalWeeks} weeks • {month.totalConsignments} consignments • £
                  ...
                  {expandedWeeks.has(week.weekKey) && (
                    <div className="mt-2 space-y-1 md:pl-6 md:border-l md:border-slate-100">
                      {week.days.map((day) => (
                        <div
                          key={day.id}
                          className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between p-3 bg-slate-50 rounded-lg"
                        >
                          <div className="flex items-center gap-3">
...
export default function HistoryPage() {
  ...
  const amountFilterId = useId();

  // Helper function to get week number of year (memoized to prevent infinite loops)
  ...
  const groupByHierarchy = useCallback(
    (dayEntries: DayEntry[]): MonthData[] => {
  ...
  const handleClearHistory = useCallback(async () => {
    ...
    try {
      ...
      if (analysesResult.isFailure) {
        throw new Error(`Failed to fetch analyses: ${analysesResult.error.message}`);
      }
      ...
      if (analyses && analyses.length > 0) {
        ...
        const failures = deleteResults.filter((result) => result.isFailure);
        if (failures.length > 0) {
          console.error("Some analyses failed to delete:", failures);
          alert(
            `Warning: ${failures.length} out of ${analyses.length} analyses could not be deleted.`
          );
        } else {
      ...
      // Client-side data minimization: clear local-only data keys
      try {
  ...
  const handleExportHistory = useCallback(() => {
    ...
    const exportHistoryData: LocalStorageExportData = {
      analysisId: `history-export-${Date.now()}`,
  ...
  // Helper function to fetch user analyses with details from database (memoized to prevent infinite loops)
  const fetchUserAnalysesWithDetails = useCallback(async (userId: string) => {
    ...
    if (result.isFailure) {
      throw new Error(`Failed to load history: ${result.error.message}`);
    }

    return result.data.data || [];
  }, []); // No dependencies - analysisRepository is stable

  // Convert database analyses to day entries (memoized to prevent infinite loops)
  const convertDatabaseAnalysesToDayEntries = useCallback(
    (analyses: AnalysisWithDetails[]): DayEntry[] => {
      // Configurable debug flag (opt-in, avoids leaking sensitive values by default)
      const DEBUG_DEDUP =
        typeof process !== "undefined" &&
        (process.env.NEXT_PUBLIC_HISTORY_DEDUP_DEBUG === "1" ||
          process.env.NEXT_PUBLIC_HISTORY_DEDUP_DEBUG === "true");

      // Track unique day entries by date with metadata for deterministic selection
      const dayEntriesMap = new Map<
        string,
        {
          entry: DayEntry;
          updatedAt: number; // analysis updated_at timestamp
          createdAt: number; // analysis created_at timestamp
          hasPaid: boolean; // whether this day has a paid amount
          hasExpected: boolean; // whether this day has an expected total
        }
      >();
      ...
      const prefer = (
        ...
        // 1) Prefer entries that include an actual paid amount
        if (candidate.hasPaid !== existing.hasPaid) return candidate.hasPaid;
        // 2) Then prefer entries with expected totals
        if (candidate.hasExpected !== existing.hasExpected) return candidate.hasExpected;
        // 3) Then prefer higher consignments
        if (candidate.entry.consignments !== existing.entry.consignments)
          return candidate.entry.consignments > existing.entry.consignments;
        // 4) Then prefer higher total amount
        if (candidate.entry.totalAmount !== existing.entry.totalAmount)
          return candidate.entry.totalAmount > existing.entry.totalAmount;
        // 5) Finally prefer most recently updated analysis, then most recently created
        if (candidate.updatedAt !== existing.updatedAt)
          return candidate.updatedAt > existing.updatedAt;
        if (candidate.createdAt !== existing.createdAt)
          return candidate.createdAt > existing.createdAt;
        // Otherwise, keep existing to remain stable
        return false;
      };
      ...
      analyses.forEach((analysis) => {
        ...
        if (analysis.daily_entries && analysis.daily_entries.length > 0) {
          analysis.daily_entries.forEach((dayEntry) => {
            const dateKey = dayEntry.date; // YYYY-MM-DD format
            const expected = dayEntry.expected_total || 0;
            const paid = dayEntry.paid_amount || 0;
            const totalAmount = expected || paid || 0;
            ...
            const existing = dayEntriesMap.get(dateKey);
            if (!existing) {
              dayEntriesMap.set(dateKey, candidate);
            } else {
              duplicateDates++;
              if (prefer(existing, candidate)) {
                dayEntriesMap.set(dateKey, candidate);
                replacements++;
              }
            }
      ...
      if (DEBUG_DEDUP && (duplicateDates > 0 || replacements > 0)) {
        // Aggregate summary without sensitive amounts
        console.info(
          `[History] Deduped ${duplicateDates} duplicate date entries, replaced ${replacements}`
        );
      }

      // Convert Map to array and sort by date (most recent first)
      const uniqueDayEntries = Array.from(dayEntriesMap.values()).map((v) => v.entry);
      return uniqueDayEntries.sort(
  ...
  useEffect(() => {
    ...
    user?.id,
    processHistoryData,
    convertDatabaseAnalysesToDayEntries,
    fetchUserAnalysesWithDetails,
  ]);
  ...
  }, [handleExportHistory]); // Depend on filteredData so handleExportHistory has access to current data
  ...
  useEffect(() => {
    ...
    filtered = filtered.map((month) => {
      ...
      const calculateWeekConsignments = (week: WeekData) => week.days.reduce(sumConsignments, 0);
      const calculateWeekAmount = (week: WeekData) => week.days.reduce(sumTotalAmount, 0);

      const totalConsignments = month.weeks.reduce(
        (sum, week) => sum + calculateWeekConsignments(week),
        0
      );
      const totalAmount = month.weeks.reduce((sum, week) => sum + calculateWeekAmount(week), 0);
  ...
  const handleWeekClick = (e: React.MouseEvent | React.KeyboardEvent, week: WeekData) => {
    ...
    if (week.days.length > 0) {
      ...
      console.log(
        "🔗 History Debug - Navigating to week view:",
        firstDay.analysisId,
        startDate,
        endDate
      );
      router.push(
        `/reports?analysis=${firstDay.analysisId}&week=${week.weekKey}&start=${startDate}&end=${endDate}`
      );
    }
  };
          ...
          return (
            <div className="max-w-4xl mx-auto">
              <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-2 gap-4">
                <Card
                  className="p-4 rounded-lg border-0 shadow-sm hover:shadow-md transition-all duration-200"
                  style={{
                    background:
