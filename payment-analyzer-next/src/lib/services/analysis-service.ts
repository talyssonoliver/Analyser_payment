...
export type ParsedPDFDataTypes =
  | (RunsheetData & { type: "runsheet" })
...
export class AnalysisService {
  ...
  async createAnalysis(
    ...
    try {
      ...
      if (request.files?.length) {
        ...
        if (existingAnalysis) {
          throw new Error(
            `Duplicate analysis detected. Analysis from ${new Date(existingAnalysis.created_at).toLocaleDateString()} already exists.`
          );
        }
      }
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        if (result.parseResult.success && result.parseResult.data) {
          ...
          const entries = this.convertToDailyEntries(
            result.parseResult.data as ParsedPDFDataTypes,
            paymentRules
          );
          ...
          console.warn(
            ...
            result.parseResult.error || "Unknown error"
          );
        }
      } catch (error) {
   ...
   */
  private processRunsheetData(
    consignments: Map<string, number> | Record<string, number>,
    paymentRules: PaymentRules
  ): DailyEntry[] {
    const entries: DailyEntry[] = [];

    const consignmentEntries =
      consignments instanceof Map
        ? Array.from(consignments.entries())
        : Object.entries(consignments);
   ...
   */
  private createDailyEntryFromRunsheet(
    date: Date,
    count: number,
    paymentRules: PaymentRules
  ): DailyEntry {
  ...
  private processInvoiceData(
    ...
    for (const payment of payments) {
      ...
      const existingEntry = entries.find((e) => e.date.toDateString() === date.toDateString());
  ...
  private processPickupServices(
    ...
    for (const pickup of pickupServices) {
      ...
      if (existingEntry) {
        ...
        // NOTE: When creating from invoice-only (no runsheet), set rate to 0
        // to prevent phantom expected amounts. The pickup amount goes to pickupTotal
        // and paidAmount should reflect the actual payment received.
        console.log(
          "🔧 [PICKUP FIX] Creating pickup entry with rate: 0, pickup amount:",
          pickup.amount
        );
        entries.push(
          new DailyEntry({
            ...
            date,
            consignments: 0,
            rate: 0, // No runsheet data = no rate-based expected amount
            paidAmount: pickup.amount, // The actual payment received for the pickup
            pickups: 1,
   ...
   * Preserves the existing rate and consignment data - only updates paid amount
   */
  private updateEntryWithPayment(
    entries: DailyEntry[],
    existingEntry: DailyEntry,
    payment: InvoiceEntry,
    _paymentRules: PaymentRules
  ): void {
    const index = entries.indexOf(existingEntry);
    const currentPaid = existingEntry.paidAmount;

    // Add payment amount to existing paid amount
    const newPaidAmount = currentPaid.amount + payment.amount;

    // Preserve existing rate and consignment data - don't recalculate from payment rules
    entries[index] = new DailyEntry({
      analysisId: "", // Will be set later
      date: existingEntry.date,
      consignments: existingEntry.consignments.count,
      rate: existingEntry.rate.amount, // Preserve existing rate (might be 0 for invoice-only)
      paidAmount: newPaidAmount,
      pickups: existingEntry.pickups?.count || 0,
      pickupTotal: existingEntry.pickupTotal?.amount || 0, // Preserve pickup total
      unloadingBonus: existingEntry.unloadingBonus.amount,
      attendanceBonus: existingEntry.attendanceBonus.amount,
      earlyBonus: existingEntry.earlyBonus.amount,
    });
   ...
   * NOTE: When creating entries from invoice-only data (no runsheet),
   * we set rate to 0 to prevent phantom expected amounts from being calculated.
   * Expected amounts should ONLY come from runsheet consignment data.
   */
  private createDailyEntryFromInvoice(
    date: Date,
    amount: number,
    _paymentRules: PaymentRules
  ): DailyEntry {
    console.log(
      "🔧 [INVOICE-ONLY FIX] Creating entry from invoice with rate: 0, paidAmount:",
      amount
    );
    return new DailyEntry({
      analysisId: "", // Will be set later
      date,
      consignments: 0,
      rate: 0, // Set to 0 - no runsheet data means no expected amount
      paidAmount: amount,
      pickups: 0,
      unloadingBonus: 0,
      attendanceBonus: 0,
  ...
  private async saveAnalysisToDatabase(
    ...
    const entriesResult = await analysisRepository.createDailyEntries(analysisId, dailyEntries);
    ...
    const totalsResult = await analysisRepository.createAnalysisTotals(analysisId, {
    ...
    if (fileRecords.length > 0) {
      const filesResult = await analysisRepository.createAnalysisFiles(analysisId, fileRecords);
      if (filesResult.isFailure) {
        throw new Error(filesResult.error.message || "Failed to create file records");
      }
    }
  ...
  async getAnalysisById(
    ...
    try {
      ...
      if (result.isFailure) {
        return { analysis: null, error: result.error.message };
      }
  ...
  async getUserAnalyses(
    ...
    const result = await analysisRepository.getUserAnalyses(userId, options);
    if (result.isFailure) {
      return { data: [], error: result.error.message };
    }
    return { data: result.data.data, error: undefined };
  }
  ...
  async getAnalyticsData(userId: string) {
    const result = await analysisRepository.getAnalyticsData(userId);
    if (result.isFailure) {
  ...
  async deleteAnalysis(analysisId: string) {
    const result = await analysisRepository.deleteAnalysis(analysisId);
    if (result.isFailure) {
