...
export class FileFingerprintService {
  ...
  static async generateHash(file: File): Promise<string> {
    try {
      ...
      return await FileFingerprintService.generateHashInChunks(file);
    } catch (error) {
      console.error("Hash generation failed:", error);
      // Fallback to simple hash based on file properties
      return FileFingerprintService.generateSimpleHash(file);
    }
  }
   ...
   */
  static async createFingerprint(file: File, analysisId?: string): Promise<FileFingerprint> {
    const hash = await FileFingerprintService.generateHash(file);
    const type = FileFingerprintService.detectFileType(file.name);
   ...
   */
  static async createFingerprints(files: File[], analysisId?: string): Promise<FileFingerprint[]> {
    const fingerprints: FileFingerprint[] = [];

    for (const file of files) {
      try {
        const fingerprint = await FileFingerprintService.createFingerprint(file, analysisId);
  ...
  static saveFingerprints(fingerprints: FileFingerprint[]): void {
    try {
      const existing = FileFingerprintService.loadAllFingerprints();
      const combined = [...existing, ...fingerprints];

      // Keep only the most recent fingerprints (prevent storage bloat)
      const sorted = combined.toSorted((a, b) => b.processedAt - a.processedAt);
      const limited = sorted.slice(0, FileFingerprintService.MAX_FINGERPRINTS);

      localStorage.setItem(FileFingerprintService.FINGERPRINTS_KEY, JSON.stringify(limited));
  ...
  static loadAllFingerprints(): FileFingerprint[] {
    try {
      const data = localStorage.getItem(FileFingerprintService.FINGERPRINTS_KEY);
  ...
  static findExistingFingerprint(file: File): FileFingerprint | null {
    const fingerprints = FileFingerprintService.loadAllFingerprints();

    // First, try to find by exact match (name, size, lastModified)
    let existing = fingerprints.find(
      (fp) =>
        fp.name === file.name && fp.size === file.size && fp.lastModified === file.lastModified
    );

    // Fallback: find by name and size only (for modified files)
    existing ??= fingerprints.find((fp) => fp.name === file.name && fp.size === file.size);
  ...
  static async compareWithExisting(file: File): Promise<FileComparison> {
    const existing = FileFingerprintService.findExistingFingerprint(file);
  ...
  static async validateFileSet(files: File[]): Promise<FingerprintValidation> {
    ...
    const currentFingerprints = await FileFingerprintService.createFingerprints(files);

    // Create lookup map for O(1) access
    const fingerprintsByName = new Map(currentFingerprints.map((fp) => [fp.name, fp]));
    ...
    for (const file of files) {
      ...
      const currentFingerprint = fingerprintsByName.get(file.name);

      // Skip if fingerprint wasn't created (shouldn't happen)
      if (!currentFingerprint) continue;

      if (comparison.isDuplicate && comparison.previousFingerprint) {
        duplicates.push({
          current: currentFingerprint,
          existing: comparison.previousFingerprint,
          type: "identical",
        });
        const processedDate = comparison.previousFingerprint.processedAt
          ? new Date(comparison.previousFingerprint.processedAt).toLocaleDateString()
          : "unknown date";
        warnings.push(`File "${file.name}" was already processed on ${processedDate}`);
        ...
        warnings.push(
          `File "${file.name}" has been modified since last analysis (${comparison.changeType})`
        );
      }
    }
  ...
  static async generateFileSetSignature(files: File[]): Promise<string> {
    const fingerprints = await FileFingerprintService.createFingerprints(files);
  ...
  static async findMatchingAnalysis(files: File[]): Promise<{
    ...
    const currentSignature = await FileFingerprintService.generateFileSetSignature(files);
    const allFingerprints = FileFingerprintService.loadAllFingerprints();
    ...
    for (const [analysisId, fingerprints] of analysisSets) {
      const analysisHashes = fingerprints
        .map((fp) => fp.hash)
        .toSorted((a, b) => a.localeCompare(b))
        .join("-");
    ...
    for (const [analysisId, fingerprints] of analysisSets) {
      let matchCount = 0;
      const currentHashes = (await FileFingerprintService.createFingerprints(files)).map(
        (fp) => fp.hash
      );
  ...
  static cleanOldFingerprints(daysOld: number = 30): void {
    try {
      const all = FileFingerprintService.loadAllFingerprints();
      const cutoff = Date.now() - daysOld * 24 * 60 * 60 * 1000;
      const recent = all.filter((fp) => fp.processedAt > cutoff);

      localStorage.setItem(FileFingerprintService.FINGERPRINTS_KEY, JSON.stringify(recent));
  ...
  static getStatistics(): {
    ...
    const uniqueAnalyses = new Set(
      fingerprints.filter((fp) => fp.analysisId).map((fp) => fp.analysisId)
    );

    const typeBreakdown = fingerprints.reduce(
      (acc, fp) => {
        if (fp.type === "runsheet") {
          acc.runsheets++;
