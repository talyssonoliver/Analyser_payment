...
export class AnalyticsService {
  ...
  async getAnalyticsData(options: AnalyticsOptions): Promise<Result<AnalyticsData>> {
    try {
      ...
      if (currentResult.isFailure) {
        return Result.failure(
          new AppError(
            ...
            500,
            false,
            { originalError: currentResult.error.message }
          )
        );
      }
      ...
      const currentAnalyses = currentResult.data.data || [];
      const previousAnalyses = previousResult.isSuccess ? previousResult.data.data || [] : [];
      ...
      return Result.failure(
        new AppError("Failed to fetch analytics data", ErrorCodes.INTERNAL_ERROR, 500, false, {
  ...
  private async calculateKPIs(
    currentAnalyses: AnalysisWithDetails[],
    previousAnalyses: AnalysisWithDetails[]
  ): Promise<KPIData[]> {
    ...
    const kpis: KPIData[] = [
      {
        ...
        change: this.calculatePercentageChange(
          currentTotals.totalEarnings,
          previousTotals.totalEarnings
        ),
      ...
      },
      {
        ...
        color: "blue",
        change: this.calculatePercentageChange(
          currentTotals.workingDays,
          previousTotals.workingDays
        ),
        changeLabel: "vs previous period",
      },
      {
        ...
        value:
          currentTotals.workingDays > 0
            ? currentTotals.totalEarnings / currentTotals.workingDays
            : 0,
        format: "currency",
        icon: "trending",
        color: "purple",
      ...
      },
      {
        ...
        color: "amber",
        change: this.calculatePercentageChange(
          currentTotals.totalConsignments,
          previousTotals.totalConsignments
        ),
        changeLabel: "vs previous period",
      },
    ];
  ...
  private calculateTrends(
    ...
    return {
      ...
      consignments: this.calculatePercentageChange(
        current.totalConsignments,
        previous.totalConsignments
      ),
  ...
  private calculatePeriodTotals(analyses: AnalysisWithDetails[]) {
    return analyses.reduce(
      (totals, analysis) => {
        const analysisTotal = Array.isArray(analysis.analysis_totals)
      ...
      analyses.forEach((analysis, index) => {
        try {
          ...
          if (Number.isNaN(date.getTime())) {
            console.warn(
              `Invalid date found in analysis at index ${index}: ${analysis.period_start}, skipping`
            );
            return;
          }
          ...
          if (!groups.has(monthKey)) {
            ...
            try {
              ...
              console.warn(
                `Failed to format date ${analysis.period_start}, using fallback format:`,
                dateError
              );
  ...
  async forecastEarnings(
    ...
    try {
      ...
      const analyses = result.data.data;
      ...
      // Calculate trend
      const recentAverage =
        dailyAverages.slice(0, 10).reduce((a, b) => a + b, 0) / Math.min(10, dailyAverages.length);
