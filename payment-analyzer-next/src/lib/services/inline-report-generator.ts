...
function generateFromManualEntries(manualEntries: ManualEntry[]): InlineReportData {
...
function generateFromDatabaseAnalysis(analysis: AnalysisWithDetails): InlineReportData {
  ...
  return {
    results: results.toSorted((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()),
    totals,
    metadata: {
...
function generateWeekReportFromDatabaseAnalysis(
  ...
  weekTotals.averageDaily =
    weekTotals.workingDays > 0 ? weekTotals.expectedTotal / weekTotals.workingDays : 0;

  return {
    results: weekResults.toSorted(
...
function generateEmptyReport(): InlineReportData {
  return {
    results: [],
    totals: {
      workingDays: 0,
...
export const InlineReportGenerator = {
  generateFromManualEntries,
  generateFromDatabaseAnalysis,
  generateWeekReportFromDatabaseAnalysis,
  generateEmptyReport,
};
