...
import type { DailyEntry } from "../entities/daily-entry";
...
export class ValidationService {
  ...
  private findMissingWorkingDays(analysis: Analysis): Date[] {
    ...
    const entryDates = new Set(analysis.dailyEntries.map((entry) => entry.date.toDateString()));

    return workingDays.filter((day) => !entryDates.has(day.toDateString()));
  }
  ...
  private findLargeDiscrepancies(entries: readonly DailyEntry[]): DailyEntry[] {
    ...
    return entries.filter(
      (entry) =>
        entry.difference.amount > threshold.amount || entry.difference.amount < -threshold.amount
    );
  }
}
