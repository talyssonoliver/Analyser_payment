...
export class FileFingerprintService {
  ...
  async createFingerprint(files: FileInfo[]): Promise<FingerprintResult> {
    ...
    const fileHashes = await Promise.all(sortedFiles.map((file) => this.createFileHash(file)));
  ...
  private createLegacyFingerprint(files: FileInfo[]): string {
    ...
    const base64 =
      typeof btoa !== "undefined" ? btoa(jsonString) : Buffer.from(jsonString).toString("base64");
  ...
  private getFileType(fileName: string): string {
    ...
    if (lowerName.includes("self") || lowerName.includes("invoice") || lowerName.includes("bill")) {
      return "invoice";
    }
