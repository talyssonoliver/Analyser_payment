...
import type { AnalysisStatus, PaymentStatus } from "../../constants";
...
export class Analysis {
  ...
  get dailyEntries(): ReadonlyArray<DailyEntry> {
    return [...this._dailyEntries];
  }
  get metadata(): Readonly<AnalysisMetadata> {
  ...
  get baseTotal(): Money {
    return this._dailyEntries.reduce((total, entry) => total.add(entry.basePayment), Money.zero());
  }

  get bonusTotal(): Money {
    return this._dailyEntries.reduce((total, entry) => total.add(entry.totalBonus), Money.zero());
  }

  get pickupTotal(): Money {
    return this._dailyEntries.reduce((total, entry) => total.add(entry.pickupTotal), Money.zero());
  }
  ...
  get paidTotal(): Money {
    return this._dailyEntries.reduce((total, entry) => total.add(entry.paidAmount), Money.zero());
  }
  ...
  getDailyEntry(date: Date): DailyEntry | undefined {
    return this._dailyEntries.find((entry) => entry.date.toDateString() === date.toDateString());
  }
