...
export function StorageAdmin() {
  ...
  const handleClearAnalysisData = () => {
    if (
      window.confirm("Are you sure you want to clear all analysis data? This cannot be undone.")
    ) {
  ...
  const handleClearAllData = () => {
    if (
      window.confirm(
        "Are you sure you want to clear ALL data including preferences? This cannot be undone."
      )
    ) {
            ...
            <div>
              ...
              <ul className="text-sm text-amber-700 space-y-1">
                {health.recommendations.map((rec: string) => (
                  <li
                    key={`rec-${rec.replace(/\s/g, "-").toLowerCase()}`}
                    className="flex items-center gap-2"
                  >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm text-gray-600">
                  {stats.usage.used} /{" "}
                  {stats.usage.used.replace(/[\d.]+/, (match: string) =>
                    (
                      parseFloat(match) + parseFloat(stats.usage.available.replace(/[^\d.]/g, ""))
                    ).toString()
                  )}
                </span>
              </div>
              <Progress value={parseFloat(stats.usage.percentage)} className="h-2" />
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <Badge variant="secondary">{stats.compression.savedPercentage} saved</Badge>
              </div>
