    ...
    return (
      <div className={`space-y-6 ${className}`} style={{ isolation: "isolate", contain: "layout" }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-2 gap-4">
          {["a", "b", "c", "d"].map((id) => (
            <Card key={`stat-skeleton-${id}`}>
              <CardHeader className="pb-3">
                <div
                  className="h-4 bg-slate-200 rounded w-2/3 animate-pulse"
              ...
              <CardContent>
                <div className="space-y-2">
                  <div
                    className="h-8 bg-slate-200 rounded w-1/2 animate-pulse"
                    style={{ isolation: "isolate", contain: "layout style" }}
                  ></div>
                  <div
                    className="h-3 bg-slate-200 rounded w-3/4 animate-pulse"
                    style={{ isolation: "isolate", contain: "layout style" }}
                  ></div>
                </div>
              </CardContent>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat) => {
          ...
          return (
            <Card
              key={stat.id}
              className="bg-gradient-to-br from-slate-800 to-slate-900 border-slate-700 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 text-white"
            >
              <CardHeader className="pb-3">
