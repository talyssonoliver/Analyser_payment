...
export function KPISection({ data, className = "" }: KPISectionProps) {
  ...
  const averageDaily =
    data.averageDaily || (data.workingDays > 0 ? data.expectedTotal / data.workingDays : 0);
  const avgConsignments =
    data.workingDays > 0 ? Math.round(data.totalConsignments / data.workingDays) : 0;
  ...
  return (
    <div className={`kpi-section ${className}`}>
        ...
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center p-4 bg-slate-50 rounded-lg">
              ...
              <div className="text-lg font-bold text-slate-900">
                {formatCurrency(data.pickupTotal)}
              </div>
      ...
      <div className="text-center">
        <Badge variant={differenceStatus.variant} className="px-6 py-2 text-sm font-semibold">
