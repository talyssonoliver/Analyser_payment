...
export function ManualEntry({
  ...
  editMode = false,
  editData,
}: Readonly<ManualEntryProps>) {
  const modalTitleId = useId();
  const dateId = useId();
  const dayId = useId();
  const consignmentsId = useId();
  const baseAmountId = useId();
  const earlyArriveId = useId();
  const attendanceId = useId();
  const unloadingId = useId();
  const expectedTotalId = useId();
  const pickupsId = useId();
  ...
  useEffect(() => {
    // Extracted function to calculate expected total (reduces nesting)
    const calculateExpectedTotal = () => {
      const baseAmountInput = document.getElementById(baseAmountId) as HTMLInputElement;
      const earlyArriveInput = document.getElementById(earlyArriveId) as HTMLInputElement;
      const attendanceInput = document.getElementById(attendanceId) as HTMLInputElement;
      const unloadingInput = document.getElementById(unloadingId) as HTMLInputElement;
      const expectedTotalInput = document.getElementById(expectedTotalId) as HTMLInputElement;
    ...
    // Extracted function to calculate payments (reduces nesting)
    const calculatePayments = () => {
      const dateInput = document.getElementById(dateId) as HTMLInputElement;
      const dayInput = document.getElementById(dayId) as HTMLInputElement;
      const consignmentsInput = document.getElementById(consignmentsId) as HTMLInputElement;
      const baseAmountInput = document.getElementById(baseAmountId) as HTMLInputElement;
      const earlyArriveInput = document.getElementById(earlyArriveId) as HTMLInputElement;
      const attendanceInput = document.getElementById(attendanceId) as HTMLInputElement;
      const unloadingInput = document.getElementById(unloadingId) as HTMLInputElement;
      ...
      const consignments = parseInt(consignmentsInput?.value || "0", 10) || 0;
      ...
      // Calculate expected total after updating form fields
      calculateExpectedTotal();
    };

    // Set up event listeners
    const dateInput = document.getElementById(dateId);
    const consignmentsInput = document.getElementById(consignmentsId);
    const earlyArriveInput = document.getElementById(earlyArriveId);
    const attendanceInput = document.getElementById(attendanceId);
    const unloadingInput = document.getElementById(unloadingId);
    ...
    <>
      ...
      <div className="flex items-center justify-between p-6 border-b border-slate-200">
        <div className="flex items-center gap-3">
          ...
          <h3 id={modalTitleId} className="text-xl font-semibold text-slate-900">
        ...
        <form className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor={dateId} className="block text-sm font-medium text-slate-700 mb-2">
                  Date
                </label>
                <input
                  id={dateId}
              ...
              <div>
                <label htmlFor={dayId} className="block text-sm font-medium text-slate-700 mb-2">
                  Day Type
                </label>
                <input
                  id={dayId}
        ...
        <button
          ...
          onClick={() => {
            if (onAddEntry) {
              ...
              const dateInput = document.getElementById(dateId) as HTMLInputElement;
              const consignmentsInput = document.getElementById(consignmentsId) as HTMLInputElement;
              const pickupsInput = document.getElementById(pickupsId) as HTMLInputElement;
              const paidAmountInput = document.getElementById(paidAmountId) as HTMLInputElement;
              const baseAmountInput = document.getElementById(baseAmountId) as HTMLInputElement;
              const earlyArriveInput = document.getElementById(earlyArriveId) as HTMLInputElement;
              const attendanceInput = document.getElementById(attendanceId) as HTMLInputElement;
              const unloadingInput = document.getElementById(unloadingId) as HTMLInputElement;
              const expectedTotalInput = document.getElementById(
                expectedTotalId
              ) as HTMLInputElement;

              const entryData: ManualEntryData = {
                id: editMode
                  ? editData?.date?.toISOString().split("T")[0] || Date.now().toString()
                  : Date.now().toString(),
                date: new Date(dateInput?.value || new Date()),
                consignments: parseInt(consignmentsInput?.value || "0", 10) || 0,
                ...
                ...(editMode && {
                  ...
                  pickups: parseInt(pickupsInput?.value || "0", 10) || 0,
                  bonuses: {
