...
import { type FingerprintDisplayProps, getFileKey } from "./types";
  ...
  return (
    <div className={`fingerprint-display ${className}`}>
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="font-medium text-blue-900">File Analysis Details ({files.length})</span>
      </div>
                  ...
                  {comparison?.isDuplicate && comparison.previousFingerprint && (
                    <div className="mt-2 pt-2 border-t border-slate-200">
                      <p className="text-xs text-orange-600">
                        <span className="font-medium">Duplicate detected:</span> Previously
                        processed on{" "}
                        {new Date(comparison.previousFingerprint.processedAt).toLocaleDateString()}
                      </p>
                    ...
                    <div className="mt-2 pt-2 border-t border-slate-200">
                      <div className="grid grid-cols-1 gap-2 text-xs text-slate-500">
                        <div>
                          ...
                          {comparison.previousFingerprint.hash.slice(0, 16)}...
                        </div>
                      </div>
                    </div>
                  )}
        ...
        <div className="mt-3 pt-3 border-t border-slate-200">
          <div className="flex items-center justify-between text-xs text-slate-500">
            ...
            <div className="flex items-center gap-3">
              {getFileSummary(files, fileComparisons, getFileIndicator).map(
                ({ label, count, color }) => (
...
function getFileSummary(
  files: File[],
  _fileComparisons: Record<
    string,
    {
      isDuplicate?: boolean;
      hasChanged?: boolean;
      changeType?: string;
      previousFingerprint?: { processedAt: number; analysisId?: string; hash: string };
    }
  >,
  getFileIndicator: (file: File) => {
    icon: string;
    text: string;
    variant: string;
    description: string;
