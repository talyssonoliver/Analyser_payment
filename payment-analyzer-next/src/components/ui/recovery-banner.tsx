...
import type { RecoveryBanner as RecoveryBannerType } from "@/lib/services/session-recovery-service";
...
export function RecoveryBanner({ recovery, onRestore, onDismiss }: RecoveryBannerProps) {
  ...
  return (
    <div
      className={`recovery-banner-overlay fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isAnimating ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"
      }`}
    >
      <div
        className={`recovery-banner mx-4 mt-4 rounded-lg shadow-lg border ${
          recovery.hasRuleChanges ? "bg-orange-50 border-orange-200" : "bg-blue-50 border-blue-200"
        } overflow-hidden`}
      >
        {/* Banner Header */}
        <div
          className={`px-4 py-2 text-xs font-medium text-white ${
            recovery.hasRuleChanges ? "bg-orange-600" : "bg-blue-600"
          }`}
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-lg">{recovery.hasRuleChanges ? "⚠️" : "🔄"}</span>
              <span>Session Recovery Available</span>
            </div>
            <span className="text-xs opacity-90">{recovery.minutesAgo}m ago</span>
          </div>
        </div>
