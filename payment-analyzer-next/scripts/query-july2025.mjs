...
async function queryJuly2025Data() {
  ...
  const { data: analyses, error } = await supabase
    .from("analyses")
    .select(
      "id, period_start, period_end, status, source, working_days, total_consignments, created_at, daily_entries(date, consignments, expected_total, paid_amount)"
    )
  ...
  analyses.forEach((analysis, idx) => {
    ...
    console.log(
      `Working Days: ${analysis.working_days}, Total Consignments: ${analysis.total_consignments}`
    );
    ...
    if (analysis.daily_entries && analysis.daily_entries.length > 0) {
      ...
      analysis.daily_entries.forEach((entry) => {
        console.log(
          `  📅 ${entry.date}: consignments=${entry.consignments}, expected=£${(entry.expected_total || 0).toFixed(2)}, paid=£${(entry.paid_amount || 0).toFixed(2)}`
        );
      });

      const totals = analysis.daily_entries.reduce(
        (acc, entry) => ({
