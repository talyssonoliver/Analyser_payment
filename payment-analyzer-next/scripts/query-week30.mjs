...
async function queryWeek30Data() {
  ...
  const { data: analyses, error } = await supabase
    .from("analyses")
    .select(
      "id, period_start, period_end, status, source, working_days, total_consignments, daily_entries(date, consignments, expected_total, paid_amount)"
    )
  ...
  const grandTotal = { consignments: 0, expected: 0, paid: 0 };
